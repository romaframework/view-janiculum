...
this.searchfield = function(id){
	
	// Text you want to set as a default value of your search field.
	var defaultText = "Cerca...";	
	...
	window.safari = ((parseInt(navigator.productSub)>=20020000)&&(navigator.vendor.indexOf("Apple Computer")!=-1));
	if(field && !safari){
		...
		if (suggestion){
			...
			field.onkeypress = function(e){
				...
				var key = window.getKeyCode(e);
		
				if(key == 13){ // enter
					selectList(list, selectedIndex, field);
					selectedIndex = 0;
					return false;
				};	
			};
			...
			field.onkeyup = function(e){
				...
				case 40: // down
					...
					break;
				default:
					startList(field, suggestionText, list, selectedIndex);
					break;
				};
			};
...
window.clearList = function(list, selectedIndex){
	if(list){
		list.style.display = "none";
		selectedIndex = 0;
	};
...
window.startList = function(field, suggestionText, list, selectedIndex){
	var arr = getListItems(field.value, suggestionText);
	if(field.value.length > 0){
		createList(arr, list, selectedIndex, field);
	} else {
		clearList(list, selectedIndex);
	};	
};

window.getListItems = function(value, suggestionText){
	var arr = new Array();
...
window.createList = function(arr, list, selectedIndex, field){
	resetList(list);			
	if(arr.length > 0) {
		for(i=0;i<arr.length;i++){				
			...
			li.onmouseover = function(){
				navListItem(this.i, list);
			};
			a.onmousedown = function(){
				selectedIndex = this.i;
				selectList(list, this.i, field);		
				return false;
			};					
			li.appendChild(a);
...
window.resetList = function(list){
...
window.navList = function(dir, list){			
	selectedIndex += (dir == "down") ? 1 : -1;
	li = list.getElementsByTagName("li");
	if (selectedIndex < 1) selectedIndex =  li.length;
	if (selectedIndex > li.length) selectedIndex =  1;
	navListItem(selectedIndex, list);
};

window.navListItem = function(index, list){	
	selectedIndex = index;
	li = list.getElementsByTagName("li");
...
window.selectList = function(list, selectedIndex, field){
	li = list.getElementsByTagName("li");	
