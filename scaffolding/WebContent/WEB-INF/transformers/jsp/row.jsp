...
<table id="<%=janiculum.id(null)%>" class="<%=janiculum.cssClass(null)%> area-row" style="<%=janiculum.inlineStyle(null)%>" cellpadding="0" cellspacing="0">
  ...
  for(Object c:janiculum.getChildren()){
 	...
 	JspTransformerHelper.addCss(janiculum.id(null)+"_"+col, "vertical-align", janiculum.areaVerticalAlignment(child));
 	...
 	JspTransformerHelper.addCss(janiculum.id(null)+"_"+col+" > table", "margin-left", marginLeftChild);
 	JspTransformerHelper.addCss(janiculum.id(null)+"_"+col+" > table", "margin-right", marginRightChild);
 	
 	JspTransformerHelper.addCss(janiculum.id(null)+"_"+col+" > div.POJO > table.area_main", "margin-left", marginLeftChild);
 	JspTransformerHelper.addCss(janiculum.id(null)+"_"+col+" > div.POJO > table.area_main", "margin-right", marginRightChild);
 	
 	if("justify".equals(hAlignChild)){
 		JspTransformerHelper.addCss(janiculum.id(null)+"_"+col+" > table", "width", "100%");
 		JspTransformerHelper.addCss(janiculum.id(null)+"_"+col+" > div.POJO > table.area_main", "width", "100%");
 	}
  %>
      <td id="<%=janiculum.id(null)%>_<%=col%>" class="col_<%=col %>"><%JspTransformerHelper.delegate(child, null,pageContext.getOut()); %></td>
  <%
